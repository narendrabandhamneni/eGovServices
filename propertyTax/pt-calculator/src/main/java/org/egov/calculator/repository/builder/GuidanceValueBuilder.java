...
public class GuidanceValueBuilder {
    ...
    public static String getGuidanceValueSearchQuery(String tenantId, String boundary,
        ...
        searchSql.append("SELECT * FROM egpt_mstr_guidancevalue WHERE tenantId = '" + tenantId + "'"
                + "AND boundary = '" + boundary + "'  OR code = " + code
                + "AND (fromdate <" + validDate + "< todate) OR structure = '" + structure + "' OR usage = '" + usage + "' "
                + "OR subusage = '" + subUsage + "' OR occupancy = '" + occupancy + "'");
